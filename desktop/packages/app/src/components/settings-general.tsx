...
export const SettingsGeneral: Component = () => {
  ...
  const AccountSection = () => (
    <Show when={melangeAuth.available}>
      <div class="flex flex-col gap-1">
        <h3 class="text-14-medium text-text-strong pb-2">Qualcomm Melange account</h3>
        <SettingsList>
          <SettingsRow
            title="Account"
            description={(() => {
              const state = melangeAuth.state()
              return state?.status === "authenticated" ? `Signed in as ${state.account}` : "Signed out"
            })()}
          >
            <div class="flex gap-2">
              <Show
                when={melangeAuth.state()?.status === "authenticated"}
                fallback={
                  <Button size="small" variant="primary" onClick={() => void melangeAuth.loginBrowser()}>
                    Sign in
                  </Button>
                }
              >
                <Button
                  ...
                  disabled={(() => {
                    const state = melangeAuth.state()
                    return state?.status === "authenticated" && state.environmentManaged
                  })()}
              ...
              <Button size="small" variant="secondary" onClick={() => void melangeAuth.recheck()}>
                Recheck
              </Button>
            </div>
          </SettingsRow>
