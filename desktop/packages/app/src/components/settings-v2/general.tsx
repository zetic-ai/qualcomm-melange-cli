...
export const SettingsGeneralV2: Component<{
  ...
  const melangeAuth = useMelangeAuth()
  ...
  const AccountSection = () => {
    ...
    const label = () => {
      ...
      if (current.status === "authenticated") return `Signed in as ${current.account}`
      if (current.status === "unauthenticated") return "Signed out"
      return current.message
    }
    return (
      <Show when={melangeAuth.available}>
        <div class="settings-v2-section">
          <h3 class="settings-v2-section-title">Qualcomm Melange account</h3>
          <SettingsListV2>
            <SettingsRowV2 title="Account" description={label()}>
              <div class="flex items-center gap-2">
                <Show
                  when={state()?.status === "authenticated"}
                  fallback={
                    <ButtonV2
                      size="normal"
                      variant="contrast"
                      disabled={Boolean(melangeAuth.busy())}
                      onClick={() => void melangeAuth.loginBrowser()}
                    >
                      Sign in
                    </ButtonV2>
                  }
                >
                  <ButtonV2
                    size="normal"
                    variant="neutral"
                    disabled={Boolean(melangeAuth.busy()) || authenticated()?.environmentManaged}
                    onClick={() => void melangeAuth.logout()}
                  >
                    Log out
                  </ButtonV2>
                </Show>
                <ButtonV2 size="normal" variant="neutral" onClick={() => void melangeAuth.recheck()}>
                  Recheck
                </ButtonV2>
              </div>
            </SettingsRowV2>
