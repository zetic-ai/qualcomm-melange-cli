...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("melange-agent://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("melange-agent://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("melange-agent://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("melange-agent://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("melange-agent://open-project")).toBeUndefined()
    expect(parseDeepLink("melange-agent://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "melange-agent://open-project?directory=/a",
      "melange-agent://other?directory=/b",
      "melange-agent://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("melange-agent://new-session?directory=/tmp/demo")).toEqual({
      directory: "/tmp/demo",
    })
    expect(parseNewSessionDeepLink("melange-agent://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("melange-agent://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("melange-agent://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "melange-agent://new-session?directory=/a",
      "melange-agent://open-project?directory=/b",
      "melange-agent://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __OPENCODE__: {
        deepLinks: ["melange-agent://open-project?directory=/a"],
      },
    } as unknown as Window & { __OPENCODE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["melange-agent://open-project?directory=/a"])
