...
import { createMelangeAuth } from "./melange-auth"
...
const APP_NAME = "Melange Agent"
const APP_ID = "ai.zetic.melange-agent"
...
const main = Effect.gen(function* () {
  ...
  const melangeAuth = createMelangeAuth(getMelangeCommand(), process.env)

  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("melange-agent://"))
    if (urls.length) {
  ...
  app.setAsDefaultProtocolClient("melange-agent")
  registerRendererProtocol()
  setDockIcon()
  const updater: UpdaterController = {
    ...
    install: async () => {
      throw new Error("Updates are disabled in Melange Agent")
    },
  }
  const menuDeps = {
    trigger: (id: string) => {
