...
import type { MelangeAuthPlatform } from "@opencode-ai/app"
...
type Deps = {
  ...
  melangeAuth: MelangeAuthPlatform
...
export function registerIpcHandlers(deps: Deps) {
  ...
  const authenticatedSender = (event: IpcMainInvokeEvent) => {
    const win = BrowserWindow.fromWebContents(event.sender)
    if (!win || win.isDestroyed() || win.webContents !== event.sender || event.senderFrame !== event.sender.mainFrame) {
      throw new Error("Invalid Melange authentication sender")
    }
  }
  ipcMain.handle("melange-auth-status", (event) => {
    authenticatedSender(event)
    return deps.melangeAuth.status()
  })
  ipcMain.handle("melange-auth-login-browser", (event) => {
    authenticatedSender(event)
    return deps.melangeAuth.loginBrowser()
  })
  ipcMain.handle("melange-auth-login-token", (event, token: unknown) => {
    authenticatedSender(event)
    if (typeof token !== "string") throw new Error("Invalid personal access token")
    return deps.melangeAuth.loginToken(token)
  })
  ipcMain.handle("melange-auth-cancel-login", (event) => {
    authenticatedSender(event)
    return deps.melangeAuth.cancelLogin()
  })
  ipcMain.handle("melange-auth-logout", (event) => {
    authenticatedSender(event)
    return deps.melangeAuth.logout()
  })
