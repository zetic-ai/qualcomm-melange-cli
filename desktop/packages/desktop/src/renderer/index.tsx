...
const createPlatform = (windowState: DesktopWindowState): Platform => {
  ...
  return {
    ...
    melangeAuth: window.api.melangeAuth,
...
function MelangeFirstLaunchGate(props: { children: JSX.Element }) {
  const auth = useMelangeAuth()
  const [complete, controls] = createResource(async () => {
    const value = await window.api.storeGet("melange-agent.dat", "authOnboardingComplete")
    return value === "true"
  })
  const finish = async () => {
    await window.api.storeSet("melange-agent.dat", "authOnboardingComplete", "true")
    controls.mutate(true)
  }

  createEffect(() => {
    if (auth.state()?.status === "authenticated" && complete.latest === false) void finish()
  })

  const ready = () => complete.latest !== undefined && auth.state() !== undefined
  return (
    <Show when={ready()} fallback={<LoadingSplash />}>
      <Show
        when={complete.latest || auth.state()?.status === "authenticated"}
        fallback={<MelangeSignIn firstLaunch onContinue={() => void finish()} />}
      >
        {props.children}
      </Show>
    </Show>
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  function App() {
    ...
    const ready = createMemo(() => !defaultServer.loading && !sidecar.loading && !wslServers.isLoading)
