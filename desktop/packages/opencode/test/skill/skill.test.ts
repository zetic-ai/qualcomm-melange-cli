...
describe("skill", () => {
  it.live("discovers packaged Melange skills without changing user configuration", () =>
    provideTmpdirInstance(
      (dir) =>
        Effect.acquireUseRelease(
          Effect.promise(async () => {
            const root = path.join(dir, "packaged-skills")
            await Bun.write(
              path.join(root, "melange-qcom", "SKILL.md"),
              `---\nname: melange-qcom\ndescription: Packaged Qualcomm Melange skill.\n---\n\n# Melange QCOM\n`,
            )
            const previous = process.env.MELANGE_AGENT_SKILLS_DIR
            process.env.MELANGE_AGENT_SKILLS_DIR = root
            return previous
          }),
          () =>
            Effect.gen(function* () {
              const skill = yield* Skill.Service
              const item = (yield* skill.all()).find((entry) => entry.name === "melange-qcom")
              expect(item?.location).toContain(path.join("packaged-skills", "melange-qcom", "SKILL.md"))
            }),
          (previous) =>
            Effect.sync(() => {
              if (previous === undefined) delete process.env.MELANGE_AGENT_SKILLS_DIR
              else process.env.MELANGE_AGENT_SKILLS_DIR = previous
            }),
        ),
      { git: true },
    ),
